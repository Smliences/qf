...
var cityList;
function chooseProvince(pTag) {
	var index = pTag.selectedIndex - 1;
	var provinceName = provinceNameArray[index];
	for (var i = 0 ; i < provinceList.length; i++) {
		if (provinceName == provinceList[i].name) {
			
			cityTag.innerHTML = "";
			cityTag.innerHTML = '<option value= "1">请选择市</option>';
			areaTag.innerHTML = "";
			areaTag.innerHTML = '<option value="1">请选择区</option>';
			cityList = provinceList[i].cityList;
			console.log(cityList);
			for (var j = 0; j < cityList.length; j++) {
				var cityName = cityList[j].name;
				cityTag.add(new Option(cityName, j));
			}
		}
	}
}

function chooseCity(cTag) {
	var index = cTag.selectedIndex - 1;
	var cityName = cityList[index].name;
//	console.log(cityName);
	for (var k = 0; k < cityList.length; k++) {
		if (cityName == cityList[k].name) {
			areaTag.innerHTML = "";
			areaTag.innerHTML = '<option value = "1">请选择区</option>';
			areaList = cityList[k].areaList;
			console.log(areaList);
			for (var g = 0; g < areaList.length; g++) {
				var areaName = areaList[g];
				console.log(areaName);
				areaTag.add(new Option(areaName, k));
			}
		}
	}
}
